...
export default class extends Vue {
  ...
  get rules() {
    return {
      name: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            if (!value) {
              callback(new Error('请输入饮品名称'))
            } else {
              const reg = /^([A-Za-z0-9\u4e00-\u9fa5]){2,20}$/
              if (!reg.test(value)) {
                callback(new Error('饮品名称输入不符，请输入2-20个字符'))
              } else {
                callback()
              }
            }
      ...
      categoryId: [
        { required: true, message: '请选择饮品分类', trigger: 'change' }
      ],
      image: {
        required: true,
        message: '饮品图片不能为空'
